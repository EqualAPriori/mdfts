...
Attributes:
    ...
    ff_class_dict (list): dict to go from string to calling the appropriate potential
"""
...
# local imports
from . import yamlhelper as yaml
from . import parsify
...
# ===== CODE =====
def vprint(string):
    """printing that is controlled by module-level `verbose` flag
...
class ForceField:
    """
    Attributes:
        loaded_file (dict-like): from reading in file or dict
        processed_file (dict-like): all shorthand fully expanded and inferred
    Notes:
    ...
    def __init__(self, ffdef=None):
        """
        Args:
            ffdef (str, optional): ff definition. Defaults to None.
        """
        self.loaded_file = None  # loaded file should be a yaml commented map
        self.processed_file = None  # for storing fully-verbose, expanded results
    ...
    def load(self, ffdef):
        """
        Args:
            ffdef (str): filename for ff definition
        """
        if isinstance(ffdef, str):
            ...
            raise NotImplementedError("Can't load non-filename-strings")
    ...
    def save(self, prefix):
        ...
        Args:
            prefix (str):
        Note:
            resulting file name will be  `f"{prefix}_processed_ff.yaml"`
        """
        filename = "{}_processed_ff.yaml".format(prefix)
        yaml.save_dict(
            filename, self.processed_file, header="#Processed force field definition.\n"
        )

    def process_ff_dict(self):
        """Loop through force field types and expand, taking into account defaults.
        """
    ...
    def sim2md(self):
        """Convert sim-standard definition of parameters to typical MD-package definitions.

        Does so by calling specialized handler codes.
        """
        # create aliases for easy access
        loadedff = self.loaded_file
        outputff = self.processed_file
    ...
    def md2sim(self):
        """Convert typical MD-package definitions to sim-standard definition of parameters.
...
def parse_default(ffdict, outdict, section, field, fixable=True, defaultdefault=0.0):
    """default parsing function, for a *specific* mydict[section][field]  in the ff definition

    Args:
        ffdict (dict-like): a full ff-def dict to parse and expand
        outdict (dict-like): the full ff-def dict to store expanded results
        section ([type]): the section (e.g. ff-type) to address
        field (str): key to the variable being addressed
        fixable (bool, optional): whether the parameter is something that can be optimized (fixable=True) or not. 
            Defaults to True.
        defaultdefault (float, optional): The default "default" value. Defaults to 0.0.

    Returns:
        dict-like: The parsed result for the dict[section][field] requested
    ...
    if section in ffdict:
        if field in ffdict[section]:
            ...
            vprint(
                "Tried to access parameter/field {} in {}, but does not exist. Filling in with ({},{})".format(
                    field, section, defaultdefault, True
                )
            )
            parsed = {"val": defaultdefault}
            if fixable:
        ...
        vprint(
            "Tried to access parameter/field {} in {}, but does not exist.".format(
                field, section
            )
        )
        parsed = {"val": defaultdefault}
...
def fill_defaults(ffdict, defaults, paramfields, options=None):
    """Fill in default values of requested parameters.

    Args:
        ffdict (dict-like): [description]
        defaults (dict-like): dictionary of starting default values.
        paramfields (list,tuple): parameter/variable names
        options (None or (list,tuple), optional): [description]. Defaults to None.

    Returns:
        dict-like: the full force-field dict/definition
    """
...
class base_potential:
    ...
    Outlines skeleotn of what every potential must define.
    ...
    Attributes:
        fields_sim (dict): the `sim` parameter fields in a given potential
        nbody (int): number of bodies in this potential
        prefix (str): potential name prefixing every potential
    """
    ...
    def expand(cls, ffdict, outdict):
        ...
        Note:
            Required variables in defaults in e.g. bonding potential: FConst, Dist
        """
        ...
        for field, v in cls.fields_sim.items():
            sim_default[field] = parse_default(
                ffdict,
                outdict,
                section,
                field,
                fixable=v["fixable"],
                defaultdefault=v["defaultdefault"],
            )

        section = "defaults_md"
        md_default = {}
        for field, v in cls.fields_md.items():
            md_default[field] = parse_default(
                ffdict,
                outdict,
                section,
                field,
        ...
        if section in ffdict:
            if ffdict[section] is not None:
                for ientry, entry in enumerate(ffdict[section]):
                    if type(outdict[section][ientry]) is str or isinstance(
                        outdict[section][ientry], list
                    ):
                        outdict[section][ientry] = yaml.YAML.comments.CommentedMap()
                    parsed = parsify.parse_potential_entry(
                        entry,
                        cls.nbody,
                        store_dict=outdict[section][ientry],
                        prefix=cls.prefix,
                    )
                    fill_defaults(
                        outdict[section][ientry],
                        sim_default,
                        cls.fields_sim,
                        cls.options_sim,
                    )
        ...
        if section in ffdict:
            if ffdict[section] is not None:
                for ientry, entry in enumerate(ffdict[section]):
                    if (
                        type(outdict[section][ientry]) is str
                        or type(outdict[section][ientry]) is list
                    ):
                        outdict[section][ientry] = yaml.YAML.comments.CommentedMap()
                    parsed = parsify.parse_potential_entry(
                        entry,
    ...
    def convert(cls, ffdict, source, target):
        """ Convert parameters between typical MD and `sim` conventions.

        Args:
            ffdict (dict-like): 
            source (str): key to source section in dictionary
            target (str]): key to destination section in dictionary

        Raises:
            ValueError: if parameters are not defined
        ...
        if source in ffdict and ffdict[source] is not None:
            ...
            raise ValueError(
                "{} parameters not defined for potential of type {}".format(source, cls)
            )
    ...
    def sim2md(cls, ffdict):
        ...
        Raises:
            ValueError: if sim parameters are not defined
        ...
        if source in ffdict:
            ...
            raise ValueError(
                "sim parameters not defined for potential of type {}".format(cls)
            )

    @staticmethod
    def md2sim(cls, ffdict):
        """ Convert MD-convention parameters to `sim`-convention parameters
        Args:
            ffdict (dict-like: [description]

        Raises:
            ValueError: if md parameters are not defined
        ...
        if source in ffdict:
            ...
            raise ValueError(
                "md parameters not defined for potential of type {}".format(cls)
            )
...
class bond_harmonic:
    ...
    Note:
        N: sim-convention
            see sim/potential/bond.py
            U = FConst * ( r - Dist0 )^2

        N: OpenMM-HooMD
            http://docs.openmm.org/6.1.0/userguide/theory.html
            U = 0.5 * k * ( r - r0 )^2
    """
    ...
    fields_sim = OrderedDict(
        {
            "FConst": {"fixable": True, "defaultdefault": 1.0},
            "Dist0": {"fixable": True, "defaultdefault": 0.0},
        }
    )

    fields_md = OrderedDict(
        {
            "FConst": {"fixable": True, "defaultdefault": 1.0},
            "Dist0": {"fixable": True, "defaultdefault": 0.0},
        }
    ...
    def expand(cls, ffdict, outdict):
        ...
        Args:
            cls (class):
            ffdict (dict-like):
                for the bond_harmonic section of full ff
            outdict (dict-like):
                for the bond_harmonic section of full ff
    ...
    @classmethod
    def sim2md(cls, ffdict):
        """Converts sim interaction parameters to 'standard' interaction parameters. 
        
        Assume forcefields are fully defined already (e.g. filled out via expand()).
        ...
        if source in ffdict:
            for ip, potential in enumerate(ffdict[source]):
                ffdict[target][ip]["k"] = {
                    "val": 2.0 * potential["FConst"]["val"],
                    "fixed": potential["FConst"]["fixed"],
                }
    ...
    def md2sim(cls, ffdict):
        ...
        Assume forcefields are fully defined already.
        ...
        if source in ffdict:
            for ip, potential in enumerate(ffdict[source]):
                ffdict[target][ip]["FConst"] = {
                    "val": 0.5 * potential["k"]["val"],
                    "fixed": potential["k"]["fixed"],
                }
...
class pair_ljg:
    ...
    Note:
        N: sim-convention
            see sim/potential/ljgaussian.py
            ```
    ...
    fields_sim = OrderedDict(
        [
            ("B", {"fixable": True, "defaultdefault": 0.0}),
    ...
    fields_md = OrderedDict(
        [
            ("u0", {"fixable": True, "defaultdefault": 0.0}),
    ...
    def expand(cls, ffdict, outdict):
        ...
        if "a_smear" in ffdict["defaults_" + style]:
            ...
            for ientry, entry in enumerate(
                ffdict[section]
            ):  # override if Kappa wasn't defined in original ff definition
                loaded_entry = parsify.parse_potential_entry(
                    entry, cls.nbody, prefix=cls.prefix
                )
                ...
                if "Kappa" in loaded_entry and "val" in loaded_entry["Kappa"]:
                    ...
                    asmears1 = [
                        asmears[beadtype] for beadtype in loaded_entry["species"][0]
                    ]
                    asmears2 = [
                        asmears[beadtype] for beadtype in loaded_entry["species"][1]
                    ]

                    if all([a == asmears1[0] for a in asmears1]) and all(
                        [a == asmears2[0] for a in asmears2]
                    ):
                        ...
                        outdict[section][ientry]["Cut"]["val"] = (
                            5.0
                            * (0.5 * (asmears1[0] ** 2.0 + asmears2[0] ** 2.0)) ** 0.5
                        )
                        vprint(
                            "  with cutoff = 5abar = {}".format(
                                outdict[section][ientry]["Cut"]["val"]
                            )
                        )
                    else:
                        vprint(
                            "  CAUTION: Multiple bead pairs defined, but asmears are not consistent, cannot define unique Kappa for this potential. Using default.".format()
                        )
            ...
            for ientry, entry in enumerate(
                ...
                if "sigma_g" in loaded_entry and "val" in loaded_entry["sigma_g"]:
                    ...
                    if all([a == asmears1[0] for a in asmears1]) and all(
                        ...
                        vprint(
                            "  Using asmear mixing rule, sigma_g = {}".format(sigma_g)
                        )
                        outdict[section][ientry]["cut"]["val"] = (
                            5.0
                            * (0.5 * (asmears1[0] ** 2.0 + asmears2[0] ** 2.0)) ** 0.5
                        ...
                        vprint(
                            "  CAUTION: Multiple bead pairs defined, but asmears are not consistent, cannot define unique sigma_g for this potential. Using default.".format()
                        )
    ...
    def sim2md(cls, ffdict):
        ...
        if source in ffdict:
            for ip, potential in enumerate(ffdict[source]):
                ...
                ffdict[target][ip]["u0"] = {
                    "val": u0,
                    "fixed": potential["Kappa"]["fixed"],
                }
                ffdict[target][ip]["sigma_g"] = {
                    "val": sigma_g,
                    "fixed": potential["B"]["fixed"],
                }
                ffdict[target][ip]["r0"] = copy.deepcopy(potential["Dist0"])
        ...
        ffdict["defaults_md"]["a_smear"] = copy.deepcopy(
            ffdict["defaults_sim"]["a_smear"]
        )
    ...
    def md2sim(cls, ffdict):
        ...
        ffdict["defaults_sim"]["a_smear"] = copy.deepcopy(
            ffdict["defaults_md"]["a_smear"]
        )
...
class external_sin:
    ...
    fields_sim = OrderedDict(
        [
            ("UConst", {"fixable": True, "defaultdefault": 0.0}),
...
class coulomb_smeared:
    ...
    fields_sim = OrderedDict(
        [
            ("Coef", {"fixable": True, "defaultdefault": 0.0}),
    ...
    fields_md = OrderedDict(
        [
            ("lb", {"fixable": True, "defaultdefault": 0.0}),
    ...
    def expand(cls, ffdict, outdict):
        ...
        if "a_smear" in ffdict["defaults_" + style]:
            ...
            for ientry, entry in enumerate(
                ffdict[section]
            ):  # override if BornA wasn't defined in original ff definition
                loaded_entry = parsify.parse_potential_entry(
                    entry, cls.nbody, prefix=cls.prefix
                )
                vprint("entry {}, {}".format(ientry, entry))
                if "BornA" in loaded_entry and "val" in loaded_entry["BornA"]:
                    ...
                    if all([a == asmears1[0] for a in asmears1]) and all(
                        ...
                        vprint(
                            "  CAUTION: Multiple bead pairs defined, but asmears are not consistent, cannot define unique smearing for this potential. Using default.".format()
                        )
        ...
        if "a_smear" in ffdict["defaults_" + style]:
            ...
            for ientry, entry in enumerate(
                ffdict[section]
            ):  # override if SmearA wasn't defined in original ff definition
                loaded_entry = parsify.parse_potential_entry(
                    entry, cls.nbody, prefix=cls.prefix
                )
                vprint("entry {}, {}".format(ientry, entry))
                if "SmearA" in loaded_entry and "val" in loaded_entry["SmearA"]:
                    ...
                    if all([a == asmears1[0] for a in asmears1]) and all(
                        ...
                        a_smear = (
                            0.5 * (asmears1[0] ** 2.0 + asmears2[0] ** 2.0)
                        ) ** 0.5
                        outdict[section][ientry]["a_smear"]["val"] = a_smear
                        vprint(
                            "  Using asmear mixing rule, a_smear = {}".format(a_smear)
                        )
                    else:
                        vprint(
                            "  CAUTION: Multiple bead pairs defined, but asmears are not consistent, cannot define unique smearing for this potential. Using default.".format()
    ...
    def sim2md(cls, ffdict):
        ...
        if source in ffdict:
            for ip, potential in enumerate(ffdict[source]):
                ...
                ffdict[target][ip]["a_smear"]["val"] = (
                    potential["BornA"]["val"] / np.pi ** 0.5
                )
                ffdict[target][ip]["rcut"] = copy.deepcopy(potential["Cut"])
        else:
            raise ValueError(
    ...
    def md2sim(cls, ffdict):
        ...
        if source in ffdict:
            for ip, potential in enumerate(ffdict[source]):
                ...
                ffdict[target][ip]["BornA"]["val"] = (
                    potential["a_smear"]["val"] * np.pi ** 0.5
                )
...
# ===== Dictionary mappings and naming conventions =====
...
ff_class_dict = {
    ...
    "extsin": external_sin,
    "coulomb_smeared": coulomb_smeared,
    "coul_smeared": coulomb_smeared,
    "coulsmear": coulomb_smeared,
}
