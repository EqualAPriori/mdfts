...
""" Utilities for defining and carrying out mappings
Envisioned workflow:
  1) Map single chain:
...
def save(
    ...
    yaml.save_dict(
        mappingfile,
        d,
        header="# mapping specifications for molecule {}\n".format(prefix),
    )
    ...
    if mode.lower() in ["simple", "contiguous", "short"]:
        ...
        for cgindex, entry in enumerate(shorthand):
            if isinstance(entry, list) or isinstance(entry, tuple):
                ...
                assert (
                    len(entry) == 2
                ), "mapping entry for simple shorthand should be an integer or a 2-element tuple"
                cg_bead_name, num_in_cgbead = entry
            elif isinstance(entry, int):
                cg_bead_name, num_in_cgbead = "DMY", entry
            else:
                raise ValueError(
                    "Unrecognized mapping entry for simple shorthand: {}".format(entry)
                )
...
def generate_single_mapping(mode, extra=None):
    ...
    if type(mode) is str:
        if isinstance(extra, mdtraj.Topology):
            ...
            if mode.lower() in ["121", "1to1"]:
                ...
                aa_indices_in_cg = [
                    (r.name, [a.index for a in r.atoms]) for r in top.residues
                ]
            ...
            print(
                "using full custom mapping, extra input is [#atoms x 1] array specifying the CG site ID of each atom"
            )
            cg_site_of_aa = extra
            ...
            for (
                ...
                extra
            ):  # this is only really efficient if a single molecule, since it doesn't assume contiguity.
                ...
                if cgsite_identifier not in cgsites:
                    ...
                    indices_in_cg = [
                        ii
                        for ii, siteID in enumerate(extra)
                        if siteID[1] == cgsite_identifier
                    ]
            ...
            print(
                "using compact mapping mode assuming contiguous sequencing of AA beads. Extra input should be array of # AA beads in each CG bead"
            )
            aa_indices_in_cg, cg_site_of_aa = generate_single_mapping_shorthand(
                "simple", extra
            )
            """
            aa_indices_in_cg = []
            ...
            print(
                "using most compact mapping mode assuming contiguous sequencing of AA beads. Extra input should be array of # AA beads in each CG bead"
            )
            aa_indices_in_cg, cg_site_of_aa = generate_single_mapping_shorthand(
                "simplest", extra
            )
    ...
    if isinstance(extra, mdtraj.Topology):
        ...
        for cg_index, entry in enumerate(aa_indices_in_cg):
            ...
            chain_indices_in_cgbead = [
                top.atom(ii).residue.chain.index for ii in indices_in_cg
            ]
            if all(
                chain_index == chain_indices_in_cgbead[0]
                for chain_index in chain_indices_in_cgbead
            ):
                chain_indices_of_cg.append(chain_indices_in_cgbead[0])
            else:
                raise ValueError(
                    "chain indices {} of atoms in cg bead {} span more than one chain!".format(
                        chain_indices_in_cgbead, cg_index
                    )
                )
...
def generate_system_mapping(mappings):
    ...
    for mapping in mappings:
        ...
        n_beads_in_mapping = np.sum(
            [len(cgbead_mapping[1]) for cgbead_mapping in molecule_mapping]
        )  # i.e. # beads in chain/molecule

        cg_bead_names = [cgbead_mapping[0] for cgbead_mapping in molecule_mapping]
        for ii in range(n_replicates):
            new_indices = [
                (np.array(cgbead_mapping[1]) + n_atoms_total).tolist()
                for cgbead_mapping in molecule_mapping
            ]

            aa_indices_in_cg.append(
                list(zip(cg_bead_names, new_indices))
            )  # this way, can later iterate through by molecule/chain!
...
def process_pdbfile(filename):
    ...
    aa_indices_in_cg, _ = generate_single_mapping(
        mode="cg_site_of_aa", extra=cg_site_of_aa
    )
...
def process_mappingfile(filename, mode, customfield=None):
    ...
    if mode.lower() == "pdb":
        ...
        aa_indices_in_cg, cg_site_of_aa = generate_single_mapping(
            mode="aa_indices_in_cg", extra=my_mapping
        )
        shorthand = None
    elif mode.lower() in ["cg", "cg_site_of_aa", "cg_site", "cg_sites"]:
        my_mapping = mapping["cg_site_of_aa"]
        aa_indices_in_cg, cg_site_of_aa = generate_single_mapping(
            mode="cg_site_of_aa", extra=my_mapping
        )
        shorthand = None
        ...
        aa_indices_in_cg, cg_site_of_aa = generate_single_mapping_shorthand(
            mode="short", shorthand=my_mapping
        )
        ...
        if customfield is None:
            ...
            my_mapping = mapping[customfield]
        aa_indices_in_cg, cg_site_of_aa = generate_single_mapping_shorthand(
            mode="shortest", shorthand=my_mapping
        )
        shorthand = my_mapping
    else:
        print(
            "caution, trying to use a custom mode {} that may not be implemented yet"
        ).format(mode)
        if customfield is None:
            my_mapping = mapping[mode]
        else:
            my_mapping = mapping[customfield]
        aa_indices_in_cg, cg_site_of_aa, shorthand = generate_single_mapping(
            mode=mode, extra=my_mapping
        )
...
def process_mapping_system(topdef):
    ...
    for entry in system_spec:
        ...
        if mappingfile.endswith("pdb") and custom is None:
            ...
            print(
                "Assuming the pdb is the unmapped pdb, and the customlist is a shortesthand notation"
            )
            aa_indices_in_cg, _ = generate_single_mapping_shorthand(
                mode="shortest", shorthand=custom
            )
            traj_mapped = map_single(mdtraj.load(mappingfile), aa_indices_in_cg)
            ...
            if custom is None:
                print(
                    "Recognizing .yaml mapping specification for {}, using the aa_indices_in_cg section".format(
                        mappingfile
                    )
                )
                aa_indices_in_cg = chain_mapping_spec["aa_indices_in_cg"]
                traj_mapped = mdtraj.load(
                    parsify.findpath(
                        chain_mapping_spec["pdbfile_mapped"], topdef["paths"]
                    )
                )
            else:
                # (e.g. if using shorthand)
                print(
                    "Recognizing .yaml mapping specification for {}, using the {} section".format(
                        mappingfile, custom
                    )
                )
                aa_indices_in_cg, _, _ = process_mappingfile(
                    mappingfile, mode=custom, customfield=custom
                )
                traj_mapped = map_single(
                    mdtraj.load(
                        parsify.findpath(
                            chain_mapping_spec["pdbfile_unmapped"], topdef["paths"]
                        )
                    ),
                    aa_indices_in_cg,
                )
    ...
    bead_types = list(
        set([a.name for a in new_topology.atoms])
    )  # not most efficient, can optimize later
    ...
    mapped_def = yaml.YAML.comments.CommentedMap(
        [
            ("bead_types", bead_types),
            ("res_types", None),
            ("mol_types", moltype_defs),
            ("system", system_def),
        ]
    )
    # if 'paths' in topdef:
    ...
    mapped_def[
        ...
        os.getcwd()
    )  # note that in the above, the chain_files are newly generated single-chain mapped files, in case the given definition was not in .pdb format
...
def map_multiple(traj, cgtop, system_mapping):
    ...
    for ichain, chain in enumerate(system_mapping):
        ...
        for ibead, cgbead_entry in enumerate(chain):
            ...
            com = (
                np.sum(traj.xyz[:, aa_indices, :] * masses[None, :, None], 1)
                / masses.sum()
            )
...
def map_single(traj, mapping):
    ...
    for ic, cgbead_entry in enumerate(mapping):
        ...
        element = (
            mdtraj.element.virtual
        )  # for now... later on can create elements for CG simulation from cg bead name
        ...
        for jj in range(ii + 1, n_beads):
            ...
            bonded = any(
                [
                    ...
                    for a1 in aa_indices1
                    for a2 in aa_indices2
                ]
            )
            if bonded:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="topology for reading non-pdb files",
    )
    parser.add_argument(
        "-prefix",
        type=str,
        default=None,
        help="optional prefix. default is to use traj filename root.",
    )
    parser.add_argument(
        "-style",
        type=str,
        ...
        help="whether to use single-chain or system parsing",
    )
    parser.add_argument(
        "-mode",
        type=str,
        choices=["aa", "cg", "pdb", "short", "shortest"],
        default="aa",
        help="mapping specification format",
    )
    parser.add_argument(
        "-params",
        nargs="+",
        help="files specifying mappings. If more than one, can collate together into a system.",
    )
    parser.add_argument(
        "-stride", default=1, type=int, help="stride for processing trajectory"
    )
    parser.add_argument(
        "-skip", default=0, type=int, help="stride for processing trajectory"
    )
    ...
    if args.style == "single":
        if len(args.params) > 1:
            raise ValueError(
                "single mode selected, but #params more than one: {}".format(
                    args.params
                )
            )
        ...
        if filemap.endswith("pdb"):
            ...
            aa_indices_in_cg, cg_site_of_aa, shorthand = process_mappingfile(
                filemap, mode
            )
        ...
        save(
            ...
            aa_indices_in_cg,
            cg_site_of_aa,
            traj_mapped=traj_mapped,
            traj_mapping=traj_mapping,
            shorthand=shorthand,
            unmapped=args.traj,
        )
        ...
        if len(args.params) == 1:
            print(
                "1 system parameter file received, assume contains system information"
            )
            filemap = args.params[0]
            ...
            print(
                "multiple parameters received, assume is pairs of: mapping_file, # of molecule"
            )
            if len(args.params) % 2 != 0:
                raise ValueError(
                    "need even number of parameter arguments, but received odd amount"
                )
        ...
        system_aa_indices_in_cg, new_topology, mapped_def = process_mapping_system(
            system_spec
        )
        print("\n=== Do the mapping ===")
        ...
        yaml.save_dict(
            prefix + "_mapping.yaml", d, header="{} mapping summary".format(prefix)
        )
        yaml.save_dict(
            prefix + "_mapped.yaml",
            mapped_def,
            header="{} mapped system definition".format(prefix),
        )
    else:
...
if test_singlechain:
    ...
    aa_indices_in_cg, cg_site_of_aa, shorthand = process_mappingfile(
        filemap, mode, customfield="shortesthand"
    )
    traj_mapped = map_single(t, aa_indices_in_cg)
    traj_mapping = generate_pdb_mapping(t, cg_site_of_aa)
    save(
